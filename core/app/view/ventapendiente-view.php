                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="card">
                                <div class="card-header d-flex align-items-center">
                                    <h5 class="card-title mb-0 flex-grow-1">Lista de Ventas por Cobrar</h5>
                                    <div>
                                    ...
                                    <table id="customerTable" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Abono</th>
                                                <th>Costo Total</th>
                                                <th class="text-danger">Deuda x cobrar</th>
                                                <th>Cliente</th>
                                                <th>Atendido</th>
                                                <th>Fecha</th>
...
<script>
    $(document).ready(function() {
        $('#customerTable').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": "index.php?action=apiventapendiente",
            "columns": [
                { "data": "id",
                ...
                { 
                    "data": null,
                    "render": function (data, type, row) {
                        return '<button class="btn btn-primary btn-sm edit-btn" data-id="' + row.id + '"><i class="ri-coins-line align-bottom me-1"></i></button>' + ' ' + '<a href="detalleventa?tid=' + row.id + '" class="btn btn-primary btn-sm">Detalle</a>';
                    }
                }
            ],
        ...
        $('#customerTable').on('click', '.edit-btn', function() {
            let id = $(this).data('id');
            $.get("index.php?action=getventapendiente&id=" + id, function(data) {
                $('#id').text('#'+data.id);
                ...
                $('#precio').text(data.total);
                $('#preciocompra').val(data.total);
                $('#abono').text(data.total);
                $('#deuda').text(data.total-data.adelanto);
                $('#montorestante').val(data.total-data.adelanto);
